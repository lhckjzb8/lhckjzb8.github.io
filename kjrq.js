 ...
 *  ifCurrYear: true, //是否只显示当年的日历，超出后不可点击，需要设置
    ...
    holidayArray:[],//开奖日列表默认不填写时为正常周末，格式为：20171013
    workingDayArray:[]//金多宝列表，格式为：20171013
 * huangh  20171013
 */
;(function($, window, document, undefined) {
    var HolidayCalendar = function(elem, options) {
        ...
        this.defaults = {
            ...
            backToday: true,// 是否返回当天
            holidayArray:[],//开奖日列表默认不填写时为全月开奖
            workingDayArray:[]//金多宝列表
        };
    ...
    HolidayCalendar.prototype = {
        showHoverInfo: function(obj) { // hover 时显示当天信息
            ...
            this.$calendar_today
                .css({ left: offset_l-52 , top: offset_t+50 })
                .stop()
                .animate({ left: offset_l-52, top: offset_t+50, opacity: 1 });
        ...
        showCalendar: function(opts) { // 输入数据并显示
            ...
            this.$calendarDate_item.each(function(i) {
                ...
                if(day<7&&opts.holidayArray==""){
                    if (returnDateStr(new Date()) <= allDay_str) {
$itemThis.addClass('item-holiday');
} else {$itemThis.addClass('item-holiday1');}
                }
...
$itemThis.removeClass('item-holiday1');
$itemThis.addClass('item-workingDay');
} else {
$itemThis.removeClass('item-holiday');
$itemThis.removeClass('item-holiday1');
$itemThis.addClass('item-workingDay1');
}
                }
            });
        },
        ...
        renderDOM: function() { // 渲染DOM
            ...
            var _titleStr = '<div class="kjrqico"></div><a class="kjrq">开奖日期</a>' +
...
'<a href="#" class="title"></a>' +
            ...
            var _weekStr = '<li class="item">日</li>' +
                ...
                '<li class="item">四</li>' +
                '<li class="item">五</li>' +
                '<li class="item" style="margin-right:0px;">六</li>';
            var _dateStr = '';
            ...
            for (var i = 0; i < 6; i++) {
                _dateStr += '<li class="item">26</li>' +
                    ...
                    '<li class="item">26</li>' +
                    '<li class="item">26</li>' +
                    '<li class="item" style="margin-right:0px;">26</li>';
            }
             _dateStr += '<div style="color:#000;font-size: 14px;text-align: center;"><span style="color:#ccc;font-size: 18px;margin: 6px 6px 6px 0px;">○</span>已开奖<span style="color:#f43;font-size: 18px;margin: 6px 6px 6px 25px;display: inline-block;">○</span>未开奖</div>';
        ...
        inital: function() { // 初始化
            ...
            if(self.opts.hoverDate){
                self.$calendarDate_item.hover(function() {
